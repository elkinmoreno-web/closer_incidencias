 ...
 * hora: un disparo de las 07:00 UTC puede saltar a las 07:50, que en
 * verano son las 09:50 — y con una ventana de una hora justa se habría
 * perdido el aviso de ese día. Abrirla no provoca envíos dobles: la
...
export async function GET(request: NextRequest) {
  ...
  if (!forzar && !simular && (horaMadrid < 9 || horaMadrid > 11)) {
    return NextResponse.json({ omitido: true, motivo: `En Madrid son las ${horaMadrid}:00, fuera de la ventana de envío (9:00-11:59).` });
  }

  const resultado = await enviarAlertasTphDiarias({ fecha, simular, limite, ignorarFrescura: sinFrescura });
