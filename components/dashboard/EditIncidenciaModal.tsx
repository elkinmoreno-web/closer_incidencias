...
export function EditIncidenciaModal({
  ...
  function handleSave() {
    if (!motivoId) {
      setError(t('editIncidencia.selecionaMotivoError'));
      return;
    }
    setError(null);
  ...
  return (
    <>
      <button
        title={t('editIncidencia.editarCampos')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setOpen(false)}>
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-ink">{t('editIncidencia.titulo')}</h2>
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                >
                  <option value="">{t('editIncidencia.selecciona')}</option>
                  {motivos.map((m) => (
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-ink-muted">{t('editIncidencia.centro')}</label>
                <select
                  value={centroId}
                  onChange={(e) => setCentroId(e.target.value)}
                  className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-primary focus:outline-none"
                >
                  <option value="">{t('editIncidencia.sinCentro')}</option>
                  {centros.map((c) => (
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-ink-muted">{t('editIncidencia.codigoPedido')}</label>
                <input
                  value={codigoPedido}
              ...
              {!motivoRequiereDirecciones && (
                <p className="-mt-2 text-xs text-ink-muted">{t('editIncidencia.direccionesAyuda')}</p>
              )}

              <div>
                <label className="mb-1 block text-xs font-semibold text-ink-muted">{t('editIncidencia.observaciones')}</label>
                <textarea
                  value={observaciones}
              ...
              <div className="mt-2 flex justify-end gap-2">
                ...
                >
                  {pending ? t('editIncidencia.guardando') : t('editIncidencia.guardarCambios')}
                </button>
              </div>
            </div>
          </div>
