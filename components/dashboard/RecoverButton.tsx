...
import { recuperarReclamacionDePapelera } from '@/app/dashboard/reclamaciones/actions';
import { useIdioma } from '@/components/i18n/IdiomaProvider';

/** La papelera mezcla incidencias y ausencias, así que hay que saber a cuál devolver cada fila. */
export function RecoverButton({ id, tipo = 'incidencia' }: { id: string; tipo?: 'incidencia' | 'ausencia' | 'reclamacion' }) {
  const { t } = useIdioma();
  ...
  return (
    <button
      disabled={pending}
      onClick={() => startTransition(() => (tipo === 'ausencia' ? recuperarAusenciaDePapelera(id) : tipo === 'reclamacion' ? recuperarReclamacionDePapelera(id) : recuperarDePapelera(id)))}
