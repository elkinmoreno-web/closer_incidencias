...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? t('crearRider.creando') : t('crearRider.anadir')}
    </button>
  );
}
...
export function CrearRiderForm({ centros, vehiculos }: { centros: Centro[]; vehiculos: Vehiculo[] }) {
  ...
  return (
    <form action={formAction} className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
      <input name="nombre" placeholder={t('crearRider.nombreCompleto')} required className="rounded-lg border border-border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
      <input name="dni" placeholder={t('crearRider.dniNie')} required className="rounded-lg border border-border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
      <input name="email" type="email" placeholder={t('crearRider.email')} required className="rounded-lg border border-border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
      <select name="centroId" required defaultValue="" className="rounded-lg border border-border px-3 py-2 text-sm focus:border-primary focus:outline-none">
        <option value="" disabled>{t('crearRider.centroObligatorio')}</option>
        {centros.map((c) => (
      ...
      <select name="vehiculoId" defaultValue="" className="rounded-lg border border-border px-3 py-2 text-sm focus:border-primary focus:outline-none">
        <option value="">{t('crearRider.vehiculo')}</option>
        {vehiculos.map((v) => (
      ...
      {state?.success && <p className="col-span-full text-sm font-medium text-emerald-700">{t('crearRider.creado')}</p>}
