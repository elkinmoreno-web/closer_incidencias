 ...
 * STK_CFG.paramsDef del sistema de Sheets. Solo super_admin puede
 * editarlos; para el resto, se explica por qué esta pestaña está
 * vacía en vez de simplemente ocultarla sin decir nada.
 *
 * `siempreAbierto` se usa cuando este componente ES la pestaña
 * "Configuración" completa (no hace falta el acordeón); sin ese prop,
 * se comporta como un panel colapsable embebido en otra vista.
 */
export function ParametrosStockPanel({
  parametros,
  esSuperAdmin,
  onGuardado,
  ...
  return (
    <div className="rounded-card border border-border bg-surface">
      {!siempreAbierto && (
        <button onClick={() => setAbiertoManual((v) => !v)} className="flex w-full items-center justify-between gap-2 px-5 py-3.5 text-left">
      ...
      {abierto && (
        <div className={siempreAbierto ? 'p-5' : 'border-t border-border px-5 py-4'}>
